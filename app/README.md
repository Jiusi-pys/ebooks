...
- 全局搜索：通过顶部入口或 `Ctrl / ⌘ + K` 打开，按本书、学习集或全文库匹配书名、作者、章节、正文、笔记与划线批注，显示高亮片段并跳转定位。
...
输入关键词后按 Enter 或点击“搜索”。“本合集”按学习集成员限定，不按书架文件夹筛选；本书和学习集范围只纳入通过引用关联到这些书籍的笔记，独立笔记可在“文库全部内容”中找到。每个命中段落显示一条结果，最多展示前 200 条；修改关键词或范围会取消旧搜索。

搜索使用已加载的工作区数据，在浏览器内完成。没有重排正文的 PDF 从缓存原文件逐页提取文字；缺失文件、提取失败或没有文字层的页面会显示提示，不提供 OCR。详细范围、默认行为和跳转说明见根目录 [全局搜索使用说明](../README_zh.md#全局搜索)。

实现入口为 `src/components/GlobalSearch.tsx`，范围解析与匹配位于 `src/lib/search.ts`，PDF 提取位于 `src/lib/searchPdf.ts`。对应测试覆盖学习集成员、关联及独立笔记、文字定位、结果上限、取消后的过期结果和 PDF 资源释放，无新增数据库迁移。可在本目录运行：

```powershell
npm.cmd test -- src/lib/search.test.ts src/lib/searchPdf.test.ts src/components/GlobalSearch.test.ts
```

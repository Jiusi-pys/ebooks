    ...
    return (
      <div className="flex h-screen w-screen items-center justify-center bg-background">
        <div className="text-center">
          <div className="font-reading text-[40px] font-bold tracking-[0.2em] text-foreground">書房</div>
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-background text-foreground">
      ...
      <main className="min-w-0 flex-1 bg-background">
        ...
        {lib.route.view === 'reader' &&
          (book ? <ReaderView key={book.id} lib={lib} book={book} /> : <LibraryView lib={lib} />)}
        {lib.route.view === 'notes' && <NotesView lib={lib} />}
        {lib.route.view === 'note' &&
          (note ? <NoteEditor key={note.id} lib={lib} note={note} /> : <NotesView lib={lib} />)}
        {lib.route.view === 'highlights' && <HighlightsView lib={lib} />}
        {lib.route.view === 'compare' && <CompareView lib={lib} />}

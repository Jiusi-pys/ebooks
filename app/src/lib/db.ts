...
import type { Book, ChapterTranslation, Folder, Highlight, MindMap, Note } from '@/types';
...
export async function deleteBook(id: string) {
  ...
  const keys = (await d.getAllKeysFromIndex('highlights', 'by-book', id)) as string[];
...
export async function getHighlights(bookId?: string): Promise<Highlight[]> {
  const d = await db();
  const all = bookId
    ? ((await d.getAllFromIndex('highlights', 'by-book', bookId)) as Highlight[])
...
export function translationId(bookId: string, chapterId: string, targetLang: string) {
...
export async function getChapterTranslation(
  ...
  return (await (await db()).get('translations', translationId(bookId, chapterId, targetLang))) as
    | ChapterTranslation
    | undefined;
}

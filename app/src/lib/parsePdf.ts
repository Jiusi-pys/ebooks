...
export interface ParsedBook {
  ...
  cover?: string;
  chapters: Chapter[];
  /** 原始 PDF 总页数（未截断；EPUB 解析时为 undefined） */
  pageCount?: number;
}
...
async function extractPageLines(page: pdfjs.PDFPageProxy): Promise<RawLine[]> {
...
/** 渲染首页为封面缩略图 dataURL */
async function renderCover(page: pdfjs.PDFPageProxy): Promise<string | undefined> {
  try {
...
async function flattenOutline(
  ...
  for (const it of items) {
    ...
    if (depth < 1 && it.items?.length) await flattenOutline(doc, it.items, depth + 1, out);
  }
}
...
export async function parsePdf(
  ...
  const flush = () => {
    ...
    chapters.push({ id: uid(), title: curTitle || `第 ${chapters.length + 1} 节`, paragraphs: reflowLines(curLines) });
    curLines = [];
  };
  ...
  for (let p = 1; p <= pageCount; p++) {
    if (p % 5 === 0 || p === pageCount) onProgress?.('解析文字', 0.05 + 0.9 * (p / pageCount));
    // 命中目录边界 → 开新章
    if (nextBoundary < outline.length && p - 1 === outline[nextBoundary].pageIndex) {
      flush();
      curTitle = outline[nextBoundary].title;
      pendingTitleSkip = curTitle;
      nextBoundary++;
      while (nextBoundary < outline.length && outline[nextBoundary].pageIndex === p - 1) nextBoundary++;
    }
  ...
  return { title, author, cover, pageCount: doc.numPages, chapters: nonEmpty.length ? nonEmpty : chapters };
}

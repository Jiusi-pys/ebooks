...
import type { Book, Folder, Highlight, MindMap, Note, Route } from '@/types';
import {
...
export function useLibrary() {
  ...
  const reload = useCallback(async () => {
    const [b, n, h, f, m] = await Promise.all([
      getAllBooks(),
      getAllNotes(),
      getHighlights(),
      getAllFolders(),
    ...
    async (files: File[], pdfModes?: Map<File, 'reflow' | 'original'>) => {
      for (const file of files) {
        ...
        if (!isPdf && !ext.endsWith('.epub')) {
          setImports((s) => [
            ...s,
            { id: taskId, name: file.name, stage: '不支持的格式', ratio: 1, status: 'error', error: '仅支持 PDF / EPUB' },
          ]);
          continue;
        }
        setImports((s) => [...s, { id: taskId, name: file.name, stage: '排队中', ratio: 0, status: 'working' }]);
        try {
          const onProgress = (stage: string, ratio: number) =>
            setImports((s) => s.map((t) => (t.id === taskId ? { ...t, stage, ratio } : t)));
          const parsed: ParsedBook = isPdf ? await parsePdf(file, onProgress) : await parseEpub(file, onProgress);
          const readerMode = isPdf ? (pdfModes?.get(file) ?? 'reflow') : undefined;
          const book: Book = {
            id: uid(),
            title: parsed.title,
          ...
          if (isPdf) {
            ...
            await putFile({ id: book.id, type: 'pdf', data: await file.arrayBuffer() });
          }
          await putBook(book);
          emitEvent('book.imported', {
            ...
            author: book.author,
            format: book.format,
            chapters: book.chapters.map((c) => ({ id: c.id, title: c.title, paragraphs: c.paragraphs })),
          });
          setImports((s) => s.map((t) => (t.id === taskId ? { ...t, stage: '完成', ratio: 1, status: 'done' } : t)));
        } catch (e) {
          setImports((s) =>
            s.map((t) =>
              t.id === taskId
                ? { ...t, stage: '失败', ratio: 1, status: 'error', error: e instanceof Error ? e.message : '解析失败' }
                : t,
            ),
          );
        }
  ...
  const createNote = useCallback(
    async (title: string): Promise<Note> => {
      const note: Note = { id: uid(), title: title.trim() || '未命名笔记', content: '', createdAt: Date.now(), updatedAt: Date.now() };
      await putNote(note);
      await reload();
      return note;
    },
  ...
  const saveNote = useCallback(
    async (note: Note) => {
      ...
      setNotes((s) => s.map((n) => (n.id === note.id ? updated : n)).sort((a, b) => b.updatedAt - a.updatedAt));
    },
    [],
  );
  ...
  const moveBook = useCallback(async (id: string, folderId: string | undefined) => {
    setBooks((s) => {
      const book = s.find((b) => b.id === id);
      if (!book) return s;
  ...
  const createFolder = useCallback(
    async (name: string): Promise<Folder> => {
      const folder: Folder = { id: uid(), name: name.trim() || '未命名文件夹', createdAt: Date.now() };
  ...
  const saveMindMap = useCallback(async (map: MindMap) => {
    ...
    setMindMaps((s) => {
      ...
      const next = exists ? s.map((m) => (m.id === map.id ? updated : m)) : [updated, ...s];

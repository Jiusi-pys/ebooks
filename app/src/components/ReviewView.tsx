...
import { applyCloze, dueCards, formatDue, gradeCard, RATING_LABEL, type Rating } from '@/lib/srs';
...
export function ReviewView({ lib }: { lib: Library }) {
  ...
  const folderFilter = useMemo(() => {
    ...
    if (!sid) return null;
    const ids = new Set(lib.books.filter((b) => b.folderId === sid).map((b) => b.id));
    return ids;
  }, [lib.route.studySetId, lib.books]);
  const folderName = folderFilter ? lib.folders.find((f) => f.id === lib.route.studySetId)?.name : null;

  const reviewCards = useMemo(
    () => lib.highlights.filter((h) => h.review && (!folderFilter || folderFilter.has(h.bookId))),
    [lib.highlights, folderFilter],
  );
  const deckCards = deck === 'all' ? reviewCards : reviewCards.filter((h) => h.bookId === deck);
  const due = dueCards(deckCards);
  const upcoming = deckCards.filter((h) => h.review && h.review.due > Date.now()).sort((a, b) => a.review!.due - b.review!.due);

  const todayStart = new Date();
  todayStart.setHours(0, 0, 0, 0);
  const todayReviewed = reviewCards.filter(
    (h) => h.review?.lastReviewedAt && h.review.lastReviewedAt >= todayStart.getTime(),
  ).length;

  /** 有复习卡片的书 → 卡组（学习集内则只看该集的书） */
  const decks = lib.books
    .filter((b) => !folderFilter || folderFilter.has(b.id))
    .map((b) => ({ book: b, count: reviewCards.filter((h) => h.bookId === b.id).length }))
    .filter((d) => d.count > 0);

  const current: Highlight | null = queue && idx < queue.length ? (lib.highlights.find((h) => h.id === queue[idx]) ?? null) : null;
  ...
  const rate = (card: Highlight, rating: Rating) => {
    ...
    emitEvent('review.updated', { extId: card.id, rating, due: next.due, reps: next.reps, lapses: next.lapses });
    setDoneCount((n) => n + 1);
    setIdx((i) => i + 1);
    setFlipped(false);
  };
  ...
  const backToSource = (card: Highlight) =>
    lib.navigate({ view: 'reader', bookId: card.bookId, chapterId: card.chapterId, highlightId: card.id });

  const deckName = deck === 'all' ? '全部卡组' : `《${lib.books.find((b) => b.id === deck)?.title ?? ''}》`;
      ...
      <div className="mx-auto max-w-3xl px-10 pb-24 pt-12">
        <header className="mb-8 border-b border-foreground/15 pb-6">
          ...
          <h1 className="font-reading mt-2 text-[34px] font-bold tracking-wide">
            复习{folderName && <span className="ml-3 text-[16px] font-medium text-primary">学习集 · {folderName}</span>}
          </h1>
          {/* 卡组选择 */}
          <div className="mt-4 flex flex-wrap gap-1.5">
            <DeckButton active={deck === 'all'} onClick={() => { setDeck('all'); setQueue(null); }} label={`全部 ${reviewCards.length}`} />
            {decks.map(({ book, count }) => (
              <DeckButton
                key={book.id}
                active={deck === book.id}
                onClick={() => { setDeck(book.id); setQueue(null); }}
        ...
        {reviewCards.length === 0 ? (
          <div className="flex flex-col items-center py-20 text-muted-foreground">
            ...
            <p className="mt-3 text-sm">还没有复习卡片。在书摘弹层或卡片盒里点「加入复习」，把摘录变成闪卡。</p>
          </div>
        ) : queue === null ? (
          /* 卡组落地页 */
          <div>
            <div className="flex items-center justify-between rounded-lg bg-card p-5 shadow-sm">
              <div>
                <div className="font-reading text-[20px] font-semibold">{deckName}</div>
                <div className="font-meta mt-1 text-[11px] text-muted-foreground">
                  {due.length > 0 ? `${due.length} 张已到期` : '没有到期卡片'}
                  {upcoming.length > 0 && ` · 下一张 ${formatDue(upcoming[0].review!.due)}`}
                </div>
              </div>
              <button
              ...
              <div className="mt-8">
                ...
                <div className="space-y-2">
                  {upcoming.slice(0, 12).map((h) => (
                    ...
                    >
                      <span className="font-meta w-16 shrink-0 text-[10.5px] text-primary">{formatDue(h.review!.due)}</span>
                      <span className="font-reading flex-1 truncate text-[13px]">{h.text}</span>
                      <span className="font-meta shrink-0 text-[10px] text-muted-foreground">{h.chapterTitle}</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="flex flex-col items-center py-20">
            <CheckCircle2 size={30} strokeWidth={1.4} className="text-primary" />
            <p className="font-reading mt-4 text-[20px] font-semibold">本会话完成</p>
            <p className="font-meta mt-1 text-[12px] text-muted-foreground">已复习 {doneCount} 张 · {deckName}</p>
            <button
          ...
          <div>
            <div className="font-meta mb-4 flex items-center justify-between text-[11px] text-muted-foreground">
              ...
              <button onClick={() => setQueue(null)} className="hover:text-foreground">
                结束会话
              </button>
            </div>
            ...
            <div className="rounded-lg bg-card p-8 shadow-sm">
              <div className="font-meta mb-3 flex items-center gap-2 text-[10.5px] text-muted-foreground">
                <BookOpen size={11} />
                {lib.books.find((b) => b.id === current.bookId)?.title} · {current.chapterTitle}
                {(current.cloze?.length ?? 0) > 0 && <span className="text-primary">· {current.cloze!.length} 处挖空</span>}
              </div>
              <p className="font-reading min-h-24 text-[17px] leading-8">
                {flipped ? current.text : applyCloze(current.text, current.cloze)}
              </p>
              {flipped && current.note && (
                <p className="mt-4 rounded-md bg-accent/40 p-3 text-[13.5px] leading-6">{current.note}</p>
              )}
              {flipped && (
                <button onClick={() => backToSource(current)} className="font-meta mt-4 text-[11.5px] text-primary hover:underline">
                  回到原文 →
                </button>
              )}
            </div>

...
import { Languages, MessageSquarePlus, Quote, Sparkles, Underline, Highlighter, Type as TypeIcon } from 'lucide-react';
...
export function SelectionToolbar({ top, left, onHighlight, onComment, onOpenCiteBrowser, onTranslate, onAskAi, onClose }: Props) {
  ...
  const styleRow = (kind: HighlightStyle['kind'], label: string, icon: React.ReactNode) => (
      ...
      {mode === 'comment' && (
        <div className="w-[280px] p-3">
          ...
          <div className="mt-2 flex justify-end gap-2">
            <button onClick={() => setMode('menu')} className="rounded-md px-2.5 py-1 text-[12px] text-muted-foreground hover:bg-secondary">
              返回
            </button>
            <button

...
import { BookMarked, ChevronLeft, ChevronRight, FileText, Search } from 'lucide-react';
...
type Level = { depth: 1 } | { depth: 2; book: Book } | { depth: 3; book: Book; chapterIdx: number };
 ...
 */
export function CiteBrowser({ books, currentBookId, notes, onSelect, onClose }: Props) {
  ...
  const chapterList = useMemo(() => {
    if (level.depth === 1) return [] as { c: Book['chapters'][number]; i: number }[];
  ...
  const back = () => {
    if (level.depth === 3) setLevel({ depth: 2, book: level.book });
    else if (level.depth === 2) setLevel({ depth: 1 });
  };
  ...
  return (
    <div className="float-pop absolute z-40 -translate-x-1/2 rounded-lg border border-border bg-popover" onMouseDown={(e) => e.stopPropagation()}>
      <div className="w-[340px] p-3">
        {/* 头部：返回 + 面包屑 */}
        <div className="mb-2 flex items-center gap-1.5">
          {level.depth > 1 && (
            <button onClick={back} className="rounded p-0.5 text-muted-foreground hover:bg-secondary hover:text-foreground">
              <ChevronLeft size={14} />
            </button>
          )}
          <span className="font-meta truncate text-[10px] uppercase tracking-[0.16em] text-muted-foreground">{crumb}</span>
          <button onClick={onClose} className="ml-auto rounded px-1 text-[11px] text-muted-foreground hover:text-foreground">
            ✕
          </button>
        </div>
        ...
        <div className="mb-2 flex items-center gap-2">
          <span className="font-meta shrink-0 text-[10px] uppercase tracking-wider text-muted-foreground">引用到</span>
          <select
            value={noteId}
        ...
        <div className="mb-2 flex items-center gap-2 rounded-md border border-border bg-card px-2">
          ...
          <input
            ...
            placeholder={
              level.depth === 1 ? '搜索书名 / 作者…' : level.depth === 2 ? '搜索章节标题…' : '搜索文段内容…'
            }
          ...
          {level.depth === 1 &&
            (bookList.length ? (
              bookList.map((b) => (
                <button
                  key={b.id}
                  onClick={() => setLevel({ depth: 2, book: b })}
                  className="flex w-full items-center gap-2 rounded-md px-2 py-2 text-left hover:bg-secondary"
                >
                  <BookMarked size={13} className="shrink-0 text-primary/70" />
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[12.5px] font-medium">{b.title}</span>
                    <span className="font-meta block text-[10px] text-muted-foreground">
                      {b.author || '佚名'} · {b.chapters.length} 章{b.id === currentBookId ? ' · 在读' : ''}
                    </span>
                  </span>
          ...
          {level.depth === 2 &&
            (chapterList.length ? (
              chapterList.map(({ c, i }) => (
                <button
                  key={c.id}
                  onClick={() => setLevel({ depth: 3, book: level.book, chapterIdx: i })}
                  className="flex w-full items-center gap-2 rounded-md px-2 py-2 text-left hover:bg-secondary"
                >
                  ...
                  <span className="font-meta shrink-0 text-[10px] text-muted-foreground">{c.paragraphs.length} 段</span>
                  <ChevronRight size={12} className="shrink-0 text-muted-foreground" />
                </button>
              ))
            ) : (
                    ...
                    onSelect(
                      {
                        ...
                        chapterTitle: level.book.chapters[level.chapterIdx].title,
                        paraIndex: i,
                        text: p,
                      },
                      noteId,
...
function EmptyRow({ text }: { text: string }) {
  return <div className="px-2 py-6 text-center text-[11px] text-muted-foreground">{text}</div>;
}
...
function highlightMatch(text: string, q: string) {
  ...
  return (
    <>
      {text.slice(0, i)}
      <mark className="rounded-sm bg-primary/20 px-0.5 text-foreground">{text.slice(i, i + t.length)}</mark>

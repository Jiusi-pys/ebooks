...
import { CheckSquare, GraduationCap, Highlighter, MessageSquarePlus, Quote, Search, Sparkles, Tag, Trash2 } from 'lucide-react';
...
export function HighlightsView({ lib }: { lib: Library }) {
  ...
  const allTags = useMemo(() => {
    ...
    for (const h of lib.highlights) for (const t of h.tags ?? []) s.add(t);
    return [...s].sort();
  }, [lib.highlights]);

  const allColors = useMemo(() => {
    const s = new Set<string>();
    for (const h of lib.highlights) if (h.style) s.add(h.style.color);
    return [...s];
  }, [lib.highlights]);

  const booksWithCards = useMemo(
    () => lib.books.filter((b) => lib.highlights.some((h) => h.bookId === b.id)),
    [lib.books, lib.highlights],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return lib.highlights
      .filter((h) => {
        ...
        if (colorFilter !== 'all' && h.style?.color !== colorFilter) return false;
        if (typeFilter === 'mark' && (h.note || (h.aiQa?.length ?? 0) > 0)) return false;
        ...
        if (needle) {
          const hay = `${h.text} ${h.note ?? ''} ${h.name ?? ''} ${(h.tags ?? []).join(' ')}`.toLowerCase();
          if (!hay.includes(needle)) return false;
        }
        return true;
      })
      .sort((a, b) => b.createdAt - a.createdAt);
  }, [lib.highlights, q, bookFilter, tagFilter, colorFilter, typeFilter, reviewFilter]);

  const toggleReview = (h: Highlight) => {
    const next = h.review ? undefined : newReviewState();
    void lib.updateHighlight({ ...h, review: next });
    emitEvent('review.updated', { extId: h.id, inReview: !h.review, due: next?.due, review: next });
  };
  ...
  const batchReview = (join: boolean) => {
    for (const h of selectedCards) {
      if (join && !h.review) {
        ...
        void lib.updateHighlight({ ...h, review: next });
        emitEvent('review.updated', { extId: h.id, inReview: true, due: next.due, review: next });
  ...
  const batchDelete = () => {
    ...
    if (confirm(`确定删除选中的 ${n} 张卡片？`)) {
      for (const h of selectedCards) {
        ...
        emitEvent('highlight.deleted', { extId: h.id, bookTitle: lib.books.find((b) => b.id === h.bookId)?.title ?? '' });
      }
      sel.exit();
    }
  };
    ...
    <div className="h-full overflow-y-auto">
      <div className="mx-auto max-w-3xl px-10 pb-24 pt-12">
        <header className="mb-6 border-b border-foreground/15 pb-6">
          <div className="font-meta text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
            卡片盒 · {filtered.length} / {lib.highlights.length} 条
          </div>
          <h1 className="font-reading mt-2 text-[34px] font-bold tracking-wide">卡片盒</h1>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-1.5 text-[12px]">
            ...
            <select
              value={reviewFilter}
              onChange={(e) => setReviewFilter(e.target.value as 'all' | 'in' | 'out')}
              className="h-7 rounded-full border border-border bg-card px-2.5 outline-none"
            >
              ...
              {allTags.map((t) => (
                <button
                  ...
                  className={`rounded-full px-2 py-0.5 text-[11px] transition-colors ${
                    tagFilter === t ? 'bg-foreground text-background' : 'bg-secondary text-muted-foreground hover:text-foreground'
                  }`}
                >
                  {t}
                </button>
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center py-20 text-muted-foreground">
            ...
            <p className="mt-3 text-sm">
              {lib.highlights.length === 0 ? '阅读时选中文字即可划线，卡片会汇集到这里。' : '没有符合筛选条件的卡片。'}
            </p>
          </div>
        ) : (
            ...
            {filtered.map((h) => {
              ...
              return (
                <div
                  ...
                  className={`rounded-md bg-card p-4 shadow-sm ${sel.selecting ? 'cursor-pointer select-none' : ''} ${
                ...
                >
                  ...
                  <div className="mb-1.5 flex items-center gap-2">
                    ...
                    {h.name && (
                      <span className="font-meta ml-1 text-[10.5px] uppercase tracking-wider text-primary">{h.name}</span>
                    )}
                    {h.review && (
                      <span className="rounded-full bg-primary/10 px-1.5 py-px text-[10px] font-medium text-primary">复习中</span>
                    )}
                  </div>
                  ...
                  {h.note && (
                    <p className="mt-2 flex gap-1.5 rounded-md bg-accent/40 p-2.5 text-[12.5px] leading-6">
                      <MessageSquarePlus size={13} className="mt-1 shrink-0 text-primary" />
                      {h.note}
                    </p>
                  )}
                  {(h.tags?.length ?? 0) > 0 && (
                  ...
                  {(h.aiQa?.length ?? 0) > 0 && (
                    <div className="font-meta mt-2 flex items-center gap-1.5 text-[10.5px] text-primary">
                      <Sparkles size={11} /> {h.aiQa!.length} 条 AI 问答 · 最近：{h.aiQa![h.aiQa!.length - 1].q}
                    </div>
                  )}
                  <div className="font-meta mt-2 flex items-center justify-between text-[10.5px] text-muted-foreground">
                    ...
                    <span className="flex shrink-0 gap-3">
                      ...
                      <button
                        className={h.review ? 'text-primary hover:underline' : 'hover:text-primary'}
                      ...
                      {h.noteId && (
                        <button
                          ...
                          onClick={() => lib.navigate({ view: 'note', noteId: h.noteId })}
                        >
                          查看引用笔记
                        </button>
                      )}
                      <button className="hover:text-destructive" onClick={() => lib.removeHighlight(h.id)}>
                        删除
                      </button>
                    </span>
                    )}
      ...
      {sel.selecting && (
        <BatchBar
          ...
          onExit={sel.exit}
        >
          <BatchAction disabled={sel.selected.size === 0} onClick={() => batchReview(true)}>
            <GraduationCap size={13} /> 加入复习
          </BatchAction>
          <BatchAction disabled={sel.selected.size === 0} onClick={() => batchReview(false)}>
            移出复习
          </BatchAction>

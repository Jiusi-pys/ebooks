...
function BookCard({
  ...
  return (
    ...
    >
      <button onClick={() => (selecting ? onToggleSelect() : lib.openReader(book.id))} className="block w-full text-left">
        ...
        <div className="mt-3">
          <div className="truncate text-[13.5px] font-medium leading-snug" title={book.title}>
            {book.title}
          </div>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-48 bg-card">
          ...
          {book.format === 'pdf' && (
            <DropdownMenuItem
              onClick={async () => {
                const target = book.readerMode === 'original' ? 'reflow' : 'original';
                const ok = await lib.setReaderMode(book.id, target);
                if (!ok) alert('未找到该书的原始 PDF 文件（可能是旧版本导入），无法切换到原版模式。');
              }}
            >
              <BookOpenText size={13} className="mr-2" />
              {book.readerMode === 'original' ? '切换为重排文本' : '切换为原版版面'}
            </DropdownMenuItem>
          )}
          <DropdownMenuSeparator />
          ...
          {folders.map((f) => (
            ...
            >
              <FolderIcon size={13} className="mr-2 text-primary/70" /> {f.name}
              {book.folderId === f.id && <span className="ml-auto text-[10px] text-primary">当前</span>}
            </DropdownMenuItem>
          ))}
          {book.folderId && (
            <DropdownMenuItem onClick={() => lib.moveBook(book.id, undefined)}>
              <ChevronLeft size={13} className="mr-2" /> 移出文件夹
            </DropdownMenuItem>
          )}
          {folders.length === 0 && !book.folderId && (
            <div className="px-2 py-1.5 text-[11px] text-muted-foreground">还没有文件夹</div>
          )}
          <DropdownMenuSeparator />
          <DropdownMenuItem
            className="text-destructive focus:text-destructive"
            onClick={() => {
              if (confirm(`确定删除《${book.title}》？相关书摘会一并移除。`)) lib.removeBook(book.id);
            }}
          >
  ...
  return (
    ...
    >
      <button onClick={onOpen} className="block w-full text-left">
        ...
        >
          <FolderIcon size={44} strokeWidth={1.1} className="text-primary/80" fill="currentColor" fillOpacity={0.12} />
          <span className="font-meta text-[10px] uppercase tracking-wider text-muted-foreground">
            {count} 册
          </span>
        </div>
        <div className="mt-3">
          <div className="truncate text-[13.5px] font-medium leading-snug">{folder.name}</div>
          <div className="font-meta mt-1 text-[10.5px] text-muted-foreground">文件夹</div>
        </div>
      </button>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-44 bg-card">
          ...
          <DropdownMenuItem
            className="text-destructive focus:text-destructive"
            onClick={() => {
              if (confirm(`删除文件夹「${folder.name}」？其中 ${count} 本书会移回书架。`))
...
export function LibraryView({ lib }: { lib: Library }) {
  ...
  const booksIn = useMemo(
    () => (id: string | null): Book[] =>
      lib.books.filter((b) => (id ? b.folderId === id : !b.folderId || !lib.folders.some((f) => f.id === b.folderId))),
      ...
      <div className="mx-auto max-w-5xl px-10 pb-24 pt-12">
        ...
        <header className="mb-10 border-b border-foreground/15 pb-6">
          <div className="font-meta flex items-baseline justify-between text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
            <span>私人阅览室</span>
            <span>
              Vol. {String(lib.books.length).padStart(2, '0')} 册 · {lib.notes.length} 篇笔记 ·{' '}
              {lib.highlights.length} 条书摘
            </span>
          </div>
          {activeFolder ? (
            <div className="mt-3 flex items-end justify-between">
              <div>
                ...
                <h1 className="font-reading flex items-center gap-3 text-[42px] font-bold leading-tight tracking-wide">
                  <FolderIcon size={30} className="text-primary/80" fill="currentColor" fillOpacity={0.12} />
                  {activeFolder.name}
                </h1>
              </div>
              <span className="font-meta pb-2 text-[11px] text-muted-foreground">{visibleBooks.length} 册</span>
            </div>
          ) : (
            <h1 className="font-reading mt-3 text-[42px] font-bold leading-tight tracking-wide">书架</h1>
          )}
          {/* 多选开关 */}
        ...
        {!activeFolder && (
          <label
            className={`mb-12 flex cursor-pointer flex-col items-center justify-center rounded-md border border-dashed px-6 py-8 text-center transition-colors ${
              dragging ? 'border-primary bg-accent/40' : 'border-foreground/25 bg-card/50 hover:bg-card'
            }`}
          >
            <BookUp2 size={22} strokeWidth={1.6} className="text-primary" />
            <span className="mt-2 text-sm font-medium">拖入文件，或点击选择</span>
            <span className="font-meta mt-1 text-[11px] text-muted-foreground">
              支持 .pdf（含目录书签识别）与 .epub · 全在本地解析，文件不上传
            </span>
            <input
              type="file"
              accept=".pdf,.epub"
        ...
        {activeFolder && (
          <div
            ...
            className={`font-meta mb-8 flex items-center justify-center gap-2 rounded-md border border-dashed px-4 py-3 text-[11px] tracking-wider transition-colors ${
              unfileOver ? 'border-primary bg-accent/60 text-foreground' : 'border-foreground/20 text-muted-foreground'
            }`}
          >
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-12 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
            ...
            {!activeFolder && (
              ...
              >
                ...
                <span className="font-meta text-[10.5px] uppercase tracking-wider">新建文件夹</span>
              </button>
            )}
          </div>
        ) : (
          <div className="py-16 text-center text-sm text-muted-foreground">
            {activeFolder ? '这个文件夹是空的，把书拖进来或从菜单移动。' : '书架是空的。拖入一本 PDF / EPUB 开始吧。'}
          </div>
        )}
      </div>
        ...
        >
          ...
          {lib.folders.length > 0 && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="center" side="top" className="w-44 bg-card">
                {lib.folders
          ...
          {activeFolderId && lib.folders.length === 0 && (
            <BatchAction disabled={sel.selected.size === 0} onClick={() => batchMove(undefined)}>

...
import { BookOpen, Columns2, GitBranch, GraduationCap, Highlighter, Import, Layers, LibraryBig, Network, StickyNote } from 'lucide-react';
...
const NAV = [
  ...
  { view: 'compare' as const, label: '对比', icon: Columns2 },
...
export function Sidebar({ lib }: { lib: Library }) {
  const { route, navigate, books, notes, highlights, mindMaps, folders } = lib;
  ...
  const counts: Record<string, number> = {
    ...
    review: highlights.filter((h) => h.review && h.review.due <= Date.now()).length,
    studyset: folders.length,
  };
  ...
  return (
    <aside className="flex h-full w-[232px] shrink-0 flex-col border-r border-sidebar-border bg-sidebar">
      <div className="px-5 pb-5 pt-6">
        <button onClick={() => navigate({ view: 'library' })} className="text-left">
          <div className="font-reading text-[26px] font-bold leading-none tracking-wide">書房</div>
          <div className="font-meta mt-2 text-[10px] uppercase tracking-[0.16em] text-muted-foreground">
            Personal Library
          </div>
          <div className="font-meta mt-1 text-[10px] text-muted-foreground/70">NO.001 · {dateLine}</div>
        </button>
      </div>
      ...
      <nav className="px-3">
        {NAV.map(({ view, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon size={16} strokeWidth={1.8} className={active ? 'text-primary' : 'text-muted-foreground'} />
              <span className="flex-1 text-left">{label}</span>
              {counts[view] !== undefined && (
                <span className="font-meta text-[10px] text-muted-foreground/70">{counts[view]}</span>
              )}
            </button>
          );
        })}
      ...
      <div className="mt-7 flex-1 overflow-y-auto px-3">
        ...
        {books.slice(0, 6).map((b) => (
          <button
            ...
            className={`mb-1 flex w-full items-center gap-2.5 rounded-md px-2.5 py-1.5 text-left transition-colors hover:bg-sidebar-accent/60 ${
              route.view === 'reader' && route.bookId === b.id ? 'bg-sidebar-accent' : ''
            }`}
          >
            <BookCover book={b} className="h-9 w-7 shrink-0" textClass="text-[7px]" />
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[13px] leading-tight">{b.title}</span>
        ...
        {books.length === 0 && (
          <p className="px-2.5 text-xs leading-6 text-muted-foreground">书架还空着，先导入一本书吧。</p>
        )}
      </div>

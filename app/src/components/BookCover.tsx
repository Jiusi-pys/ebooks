...
export function BookCover({
  ...
  if (book.cover) {
    return (
      <div className={`overflow-hidden rounded-[3px] shadow-[0_10px_24px_-10px_rgba(62,49,32,0.45)] ${className}`}>
        <img src={book.cover} alt={book.title} className="h-full w-full object-cover" draggable={false} />
      </div>
    );
  }
  ...
  return (
    ...
    >
      {/* 书脊 */}
      <div className="absolute inset-y-0 left-0 w-[7%]" style={{ background: 'rgba(0,0,0,0.22)' }} />
      {/* 题签 */}
      <div
      ...
      >
        <span
          ...
          style={{ color: tone.bg, writingMode: 'vertical-rl', letterSpacing: '0.12em' }}
        >
          {book.title.slice(0, 9)}
        </span>
      </div>
      <div className="absolute bottom-[8%] left-[16%] right-[12%] h-px" style={{ background: tone.band, opacity: 0.7 }} />
    </div>
  );
}
